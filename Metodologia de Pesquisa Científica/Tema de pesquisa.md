...
# Projeto de pesquisa
Um documento onde é relatado todos o pontos da pesquisa e o planejamento para a execução de pesquisa, é o que é feito no TCC1
Um documento que define todos os passos da sua investigação, o planejamento da sua pesquisa
Basicamente **deve conter**: Assunto, Tema, Questão de Pesquisa, Hipótese, Objetivos, Justificativa
# Metodologia de pesquisa
O algoritmo, o conjunto de passos que serão seguidos para a produção do trabalho científico
Tomar cuidado com propostas metodológicas ingênuas

# Método de pesquisa
Deve indicar se modelos teóricos serão construídos, se protótipos serão desenvolvidos, os experimentos que serão realizados, como os dados serão organizados e comparados, testes e avaliações a serem feitos (e como deverão ser feitos)
## Etapas básicas da pesquisa
1. Descoberta do problema
	1. Colocação precisa do problema
2. Procura de conhecimentos ou instrumentos relevantes ao problema
3. Tentativa de solução ao problema
	1. Invenção de novas idéias (caso a solução inicial não seja satisfatória)
4. Obtenção de uma nova solução 
5. Investigação das consequências da solução obtida
6. Comprovação/Avaliação da solução
	1. Correção das hipóteses (caso o resultado não vá de encontro com a hipótese)

# Resultados esperados
É interessante que nas conclusões da monografia volte aos objetivos e discorra sobre quais foram alcançados e aos que não foram alcançados, porquê não foram
São diferentes dos objetivos: os objetivos devem ser verificáveis ao final do trabalho. Já os resultados esperados são apenas esperanças...

# Limitações do Trabalho
São aspectos que o autor reconhece a importância, mas não tem condições de abordar no tempo disponível
	Reconhece as limitações do trabalho desenvolvido
	É importante também estabelecer limitações desde o início, para definir um escopo, condições, em que a hipótese é verdadeira

# Partes do projeto de pesquisa
## Resumo
Parágrafo de poucas linhas em que deve-se ressaltar a problemática, os objetivos, descrever a abordagem metodológica e ressaltar os resultados obtidos
Após o resumo aparecem geralmente de 3 a 5 palavras chaves para buscas 

## Introdução
Contextualiza (aborda o tema indicando a situação e o contexto no qual o problema foi identificado)
A introdução deve introduzir o tema, justificar a importância, contextualizar a pesquisa e colocar o problema (enunciar o problema que é de interesse resolver)
Deve permitir ao leitor se "vale a pena" ler o artigo na situação dele

## Objetivos
## Justificativa
Deve-se evitar comparações com o vazio ("Não existe um método"), porquê sempre pode existir uma pesquisa que não foi achada durante a pesquisa bibliográfica por exemplo

## Referencial teórico
É uma seção para explicar, abordar, os conceitos extraídos da bibliografia que vão ser utilizados na pesquisa
Facilitar a leitura de um leitor é o principal foco, além de trazer os principais trabalhos que estão relacionados com o tópico a ser desenvolvido, determinar o estado da arte sobre o tema/problema
	É importante mencionar os autores reconhecidamente importantes ao tema escolhido
Aqui vão ser respondidas, a fim de situar em relação a o que já existe:
...
## Método de pesquisa
Deve mostrar como vai ser conduzido o trabalho de pesquisa

## Cronograma
Deve incluir as principais atividades necessárias para a conclusão do trabalho e os períodos em que elas serão realizadas

## Referências Bibliográficas
Enumerar as referências citadas ao longo do texto do Projeto de Pesquisa.
Deve ser usado um Padrão para construir a lista de referência e sua respectiva citação.
O padrão largamente usado nos textos em português é o da ABNT - Associação Brasileira de Normas Técnicas.

**Não pode citar trabalhos que não estão na lista e não pode colocar na lista sem ser citado**
# Como conduzir uma revisão bibliográfica
É recomendável que se adore um modelo envolvendo
- Planejamento de revisão, definindo como abordar o tema, as fontes de informação e as formas de se chegar às publicações
- Obtenção, seleção e leitura
- Registro de leitura (ficha de leitura)
